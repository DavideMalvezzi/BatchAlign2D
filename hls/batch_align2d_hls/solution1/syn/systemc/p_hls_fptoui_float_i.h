// ==============================================================
// RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2018.3
// Copyright (C) 1986-2018 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

#ifndef _p_hls_fptoui_float_i_HH_
#define _p_hls_fptoui_float_i_HH_

#include "systemc.h"
#include "AESL_pkg.h"


namespace ap_rtl {

struct p_hls_fptoui_float_i : public sc_module {
    // Port declarations 3
    sc_out< sc_logic > ap_ready;
    sc_in< sc_lv<32> > x;
    sc_out< sc_lv<8> > ap_return;


    // Module declarations
    p_hls_fptoui_float_i(sc_module_name name);
    SC_HAS_PROCESS(p_hls_fptoui_float_i);

    ~p_hls_fptoui_float_i();

    sc_trace_file* mVcdFile;

    sc_signal< sc_lv<32> > p_Val2_s_fu_38_p1;
    sc_signal< sc_lv<23> > tmp_V_37_fu_52_p1;
    sc_signal< sc_lv<25> > mantissa_V_fu_56_p4;
    sc_signal< sc_lv<8> > tmp_V_fu_42_p4;
    sc_signal< sc_lv<9> > tmp_i_i_i_cast1_fu_70_p1;
    sc_signal< sc_lv<9> > sh_assign_fu_74_p2;
    sc_signal< sc_lv<8> > tmp_i_i_fu_88_p2;
    sc_signal< sc_lv<1> > isNeg_fu_80_p3;
    sc_signal< sc_lv<9> > tmp_i_i_cast_fu_94_p1;
    sc_signal< sc_lv<9> > ush_fu_98_p3;
    sc_signal< sc_lv<32> > sh_assign_2_cast_fu_106_p1;
    sc_signal< sc_lv<25> > sh_assign_2_cast_cas_fu_110_p1;
    sc_signal< sc_lv<55> > mantissa_V_1_cast2_fu_66_p1;
    sc_signal< sc_lv<55> > tmp_i_i_1181_fu_114_p1;
    sc_signal< sc_lv<25> > r_V_fu_118_p2;
    sc_signal< sc_lv<1> > tmp_1505_fu_130_p3;
    sc_signal< sc_lv<55> > r_V_1_fu_124_p2;
    sc_signal< sc_lv<8> > tmp_fu_138_p1;
    sc_signal< sc_lv<8> > tmp_s_fu_142_p4;
    static const sc_logic ap_const_logic_1;
    static const bool ap_const_boolean_1;
    static const sc_lv<32> ap_const_lv32_17;
    static const sc_lv<32> ap_const_lv32_1E;
    static const sc_lv<1> ap_const_lv1_1;
    static const sc_lv<1> ap_const_lv1_0;
    static const sc_lv<9> ap_const_lv9_181;
    static const sc_lv<32> ap_const_lv32_8;
    static const sc_lv<8> ap_const_lv8_7F;
    static const sc_lv<32> ap_const_lv32_18;
    static const sc_lv<32> ap_const_lv32_1F;
    static const sc_logic ap_const_logic_0;
    // Thread declarations
    void thread_ap_ready();
    void thread_ap_return();
    void thread_isNeg_fu_80_p3();
    void thread_mantissa_V_1_cast2_fu_66_p1();
    void thread_mantissa_V_fu_56_p4();
    void thread_p_Val2_s_fu_38_p1();
    void thread_r_V_1_fu_124_p2();
    void thread_r_V_fu_118_p2();
    void thread_sh_assign_2_cast_cas_fu_110_p1();
    void thread_sh_assign_2_cast_fu_106_p1();
    void thread_sh_assign_fu_74_p2();
    void thread_tmp_1505_fu_130_p3();
    void thread_tmp_V_37_fu_52_p1();
    void thread_tmp_V_fu_42_p4();
    void thread_tmp_fu_138_p1();
    void thread_tmp_i_i_1181_fu_114_p1();
    void thread_tmp_i_i_cast_fu_94_p1();
    void thread_tmp_i_i_fu_88_p2();
    void thread_tmp_i_i_i_cast1_fu_70_p1();
    void thread_tmp_s_fu_142_p4();
    void thread_ush_fu_98_p3();
};

}

using namespace ap_rtl;

#endif
