...
typedef struct PyrRegion{
	...
	uint8_t cols;
	uint8_t rows;
	// vado a unire il concetto di regione di piramide a quella della feature
	// che si puo' trovare dentro, e' decisamente brutto da vedere, ma
	// risparmio qualche byte
	// (initial) feature position in the region
	uint8_t fcoord_x;
	uint8_t fcoord_y;
	//int region_offset_x;
	//int region_offset_y;
