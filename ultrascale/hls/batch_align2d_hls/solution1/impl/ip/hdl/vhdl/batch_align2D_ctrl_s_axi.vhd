...
-- ----------------------- Register logic ----------------
    ...
    cur_px_estimate      <= STD_LOGIC_VECTOR(int_cur_px_estimate);
    converged            <= STD_LOGIC_VECTOR(int_converged);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CUR_PX_ESTIMATE_DATA_0) then
                    int_cur_px_estimate(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_cur_px_estimate(31 downto 0));
                end if;
            end if;
        end if;
    end process;
