...
assign cur_px_estimate = int_cur_px_estimate;
assign converged       = int_converged;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
// int_cur_px_estimate[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_cur_px_estimate[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CUR_PX_ESTIMATE_DATA_0)
            int_cur_px_estimate[31:0] <= (WDATA[31:0] & wmask) | (int_cur_px_estimate[31:0] & ~wmask);
    end
end
